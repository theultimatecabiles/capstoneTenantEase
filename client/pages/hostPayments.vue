      ...
      <div v-for="(payment, index) in payments" :key="index" class="payment-container mt-6 mb-6 mx-auto max-w-4xl">
        <div class="payment-card relative p-6 bg-white shadow-md rounded-lg border-l-4" 
          ...
          <div class="flex flex-col gap-2">
            <p><strong>Amount:</strong> <span class="payment-amount text-gray-700">₱{{ payment.amount.toFixed(2) }}</span></p>
            ...
            <!-- Booking Information -->
            <p><strong>Renter:</strong> <span class="text-gray-700">{{ payment.booking.booker.name || 'Unknown Booker' }}</span></p>
            <p><strong>Payment for the Listing:</strong> <span class="text-gray-700">{{ payment.booking.listing.title || 'Unknown Listing' }}</span></p>
    ...
    setup() {
      ...
      const fetchPayments = async () => {
        try {
          ...
          const response = await axios.get(`/api/getPayments?userId=${userId}&type=host`);
