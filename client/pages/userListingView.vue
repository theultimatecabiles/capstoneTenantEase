...
// Declare these variables but don't initialize them yet
let Carousel, Slide, L;

// Function to dynamically import client-side dependencies
async function importClientDependencies() {
  if (process.client) {
    const vueCarousel = await import('vue3-carousel');
...
// ... Your other functions (prevImage, nextImage, resetAutoplay) ...
...
function handleMapClick(e) {
  if (routingControl) {
    ...
    routingControl.setWaypoints([
      L.latLng(lat, lng),
      L.latLng(listing.value.latitude, listing.value.longitude)
    ]);
  }
}

onMounted(async () => {
  await importClientDependencies();
  checkLoginStatus();
